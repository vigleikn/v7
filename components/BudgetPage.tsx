...
export const BudgetPage: React.FC<BudgetPageProps> = ({ onNavigate }) => {
  ...
  const [copyTargetMonth, setCopyTargetMonth] = useState<string | null>(null);
  ...
  const previousMonthHasBudget = useCallback(
    (month: string): boolean => {
      const prev = shiftMonth(month, -1);
      return editableCategoryIds.some((catId) => {
        const val = budgetMap.get(`${catId}|${prev}`);
        return val !== undefined && val > 0;
      });
    },
    [editableCategoryIds, budgetMap]
  );

  const copyBudgetFromPreviousMonth = useCallback(
    (targetMonth: string) => {
      const prev = shiftMonth(targetMonth, -1);
      editableCategoryIds.forEach((catId) => {
        const val = budgetMap.get(`${catId}|${prev}`);
        if (val !== undefined && val > 0) {
          setBudget(catId, targetMonth, val);
        }
      });
      setCopyTargetMonth(null);
    },
    [editableCategoryIds, budgetMap, setBudget]
  );
  ...
  const renderRow = (row: BudgetRowView, level = 0): React.ReactNode => {
    ...
    const shouldRedact = redactSensitive && (
      row.categoryId === '__expenses_total' || incomeCategoryIds.has(row.categoryId)
    );

    const isSummaryRow = row.categoryId === '__balance_row' || row.categoryId === 'cat_inntekter_default' || row.categoryId === '__expenses_total';
    const baseRowColor =
    ...
    const rowBody = (
      <>
          <tr
          className={`${rowClassNames} ${hasChildren ? 'cursor-pointer' : ''} ${isSummaryRow ? 'h-[44px]' : 'h-[36px]'}`}
          onClick={() => {
            if (hasChildren) {
        ...
        >
          <td
            className="px-2 py-1 text-sm font-medium text-gray-800 max-w-[10rem]"
          ...
          {row.monthly.map((cell, cellIndex) => {
            ...
            const isIncomeCategory = incomeCategoryIds.has(row.categoryId);
            const saldoClass = cell.saldo < 0
              ? 'text-red-600 font-medium'
              : cell.saldo > 0
                ? 'text-green-600 font-medium'
                : 'text-gray-500 font-medium';
            const currentMonthBg = isCurrentMonth ? 'bg-blue-100/70' : '';
            const isFirstColumnInMonth = cellIndex % 3 === 0;
            const monthIndex = Math.floor(cellIndex / 3);
            const monthSpacing = isFirstColumnInMonth && monthIndex > 0 ? 'border-l-2 border-gray-200 pl-3' : '';
            ...
            return (
              <React.Fragment key={key}>
                <td className={`px-2 py-1 text-right align-middle min-w-[4.5rem] w-[6rem] ${currentMonthBg} ${monthSpacing}`}>
                    ...
                    {isEditable ? (
                      activeEditingKey === key ? (
                        ...
                        <button
                          ...
                          className="w-full text-right block text-sm text-gray-500 font-medium px-2 py-2 rounded-md border border-transparent hover:border-gray-300 transition-colors"
                        >
                          <RedactedValue value={formatCurrency(cell.budget)} isRedacted={shouldRedact} />
                        </button>
                      )
                    ) : (
                ...
                <td 
                  className={`px-2 py-1 text-right align-middle text-sm min-w-[4.5rem] ${actualClass} ${currentMonthBg} relative ${
                    isCellClickable 
                ...
                >
                  <RedactedValue value={formatCurrency(cell.actual)} isRedacted={shouldRedact} />
                  {cell.budget > 0 && 
                   row.categoryId !== '__balance_row' && 
                   row.categoryId !== 'cat_inntekter_default' && (
                    <>
                      ...
                      {(() => {
                        ...
                        const progressColor = isIncomeCategory
                          ? (cell.actual < cell.budget ? 'bg-red-500' : 'bg-green-500')
                          : (cell.actual >= cell.budget ? 'bg-red-500' : 'bg-green-500');
                ...
                <td className={`px-2 py-1 text-right align-middle text-sm min-w-[4.5rem] ${saldoClass} ${currentMonthBg}`}>
                  <RedactedValue
                    value={isIncomeCategory && cell.saldo > 0 
    ...
    if (row.categoryId === '__balance_row') {
      return (
        <React.Fragment key={row.categoryId}>
          {rowBody}
          <tr className="bg-gray-50/50 border-b border-gray-100 h-[28px]">
            <td className="px-2 py-0.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">Inntekter</td>
            {visibleMonths.map((month, monthIndex) => {
              const isCurrentMonth = month === currentMonth;
              const monthSpacing = monthIndex > 0 ? 'border-l-2 border-gray-200 pl-3' : '';
              return (
                <React.Fragment key={`balance-subheader-${month}`}>
                  <td className={`px-2 py-0.5 text-right text-xs font-medium text-gray-400 min-w-[4.5rem] ${isCurrentMonth ? 'bg-blue-100/70' : ''} ${monthSpacing}`}>
                    Budsjett
                  </td>
                  <td className={`px-2 py-0.5 text-right text-xs font-medium text-gray-400 min-w-[4.5rem] ${isCurrentMonth ? 'bg-blue-100/70' : ''}`}>
                    Faktisk
                  </td>
                  <td className={`px-2 py-0.5 text-right text-xs font-medium text-gray-400 min-w-[4.5rem] ${isCurrentMonth ? 'bg-blue-100/70' : ''}`}>
                    Avvik
    ...
    if (row.categoryId === '__expenses_total') {
      return (
        <React.Fragment key={row.categoryId}>
          <tr className="bg-gray-50/50 border-t border-gray-200 h-[28px]">
            <td className="px-2 py-0.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">Utgifter</td>
            {visibleMonths.map((month, monthIndex) => {
              const isCurrentMonth = month === currentMonth;
              const monthSpacing = monthIndex > 0 ? 'border-l-2 border-gray-200 pl-3' : '';
              return (
  ...
  return (
    <div className="flex h-screen bg-gray-50">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            <table className="w-full text-sm">
              <thead className="bg-gray-100 border-b border-gray-200">
                <tr>
                  <th
                    className="px-2 py-3 text-left font-semibold text-gray-700 align-middle max-w-[10rem]"
                    scope="col"
                  >
                    <button
                      onClick={handlePrev}
                  ...
                  {visibleMonths.map((month, index) => {
                    ...
                    return (
                      <th
                        key={`month-${month}`}
                        colSpan={3}
                        className={`px-3 py-3 text-center font-semibold text-gray-700 relative ${isCurrentMonth ? 'bg-blue-100/70 border-l-2 border-r-2 border-blue-200' : ''} ${index > 0 ? 'border-l-2 border-gray-200' : ''}`}
                        scope="colgroup"
                      >
                        <span>{formatMonthHeader(month)}</span>
                        {previousMonthHasBudget(month) && (
                          <button
                            onClick={() => setCopyTargetMonth(month)}
                            className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs text-gray-500 hover:text-gray-800 hover:bg-gray-200 transition-colors"
                            title={`Kopier budsjett fra ${formatMonthHeader(shiftMonth(month, -1))}`}
                          >
                ...
                <tr className="bg-white border-t border-gray-100 h-[28px]">
                  <th className="px-2 py-0.5 text-xs font-semibold text-gray-400 text-left" />
                  {visibleMonths.map((month, monthIndex) => {
                    const isCurrentMonth = month === currentMonth;
                    const monthSpacing = monthIndex > 0 ? 'border-l-2 border-gray-200 pl-3' : '';
                    return (
                      <React.Fragment key={`summary-${month}`}>
                        <th className={`px-2 py-0.5 text-xs font-medium text-gray-400 text-right min-w-[4.5rem] ${isCurrentMonth ? 'bg-blue-100/70' : ''} ${monthSpacing}`}>
                          Budsjett
                        </th>
                        <th className={`px-2 py-0.5 text-xs font-medium text-gray-400 text-right min-w-[4.5rem] ${isCurrentMonth ? 'bg-blue-100/70' : ''}`}>
                          Faktisk
                        </th>
                        <th className={`px-2 py-0.5 text-xs font-medium text-gray-400 text-right min-w-[4.5rem] ${isCurrentMonth ? 'bg-blue-100/70' : ''}`}>
                          Avvik
      ...
      <AlertDialog open={!!copyTargetMonth} onOpenChange={(open) => { if (!open) setCopyTargetMonth(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Kopier budsjett?</AlertDialogTitle>
            <AlertDialogDescription>
              {copyTargetMonth && (
                <>
                  Budsjett fra <strong>{formatMonthHeader(shiftMonth(copyTargetMonth, -1))}</strong> kopieres
                  til <strong>{formatMonthHeader(copyTargetMonth)}</strong>.
                  Eksisterende budsjettverdier blir overskrevet.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setCopyTargetMonth(null)}>
              Avbryt
            </AlertDialogCancel>
            <AlertDialogAction onClick={() => copyTargetMonth && copyBudgetFromPreviousMonth(copyTargetMonth)}>
              Kopier
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
